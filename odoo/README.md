...
We added a custom module called "basic" to faciliate running odoo kernel light version. 

It is better to install "basic" module instead of "base" module for the first time when initializing the database. 

Just run these commands in the root directory of this repository:

```shell
cd odoo;

./odoo-bin --addons-path="./addons,./odoo/addons" -d odoo_db -i basic
```
